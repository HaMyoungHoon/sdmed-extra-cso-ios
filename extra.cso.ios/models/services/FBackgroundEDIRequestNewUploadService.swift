...
class FBackgroundEDIRequestNewUploadService {
    ...
    var ediRequestService = FDI.ediRequestService
    ...
    private func checkSASKeyQ(_ data: EDISASKeyQueueModel?) {
        ...
        resultQ.uuid = uuid
        resultQ.itemIndex = -1
        resultQ.itemCount = data.ediUploadModel.pharmaList.flatMap { $0.uploadItems }.count
    ...
    private func postResultData(_ data: EDIFileResultQueueModel) {
        ...
        Task {
            let ret = await ediRequestService.postNewData(data.parseEDIUploadModel())
            if ret.result == true {
                let ediPK = ret.data?.thisPK ?? ""
                notificationCall(FAppLocalString.ediFileUploadComp, ediPK)
                _ = await mqttService.postEDIRequest(ediPK, data.ediUploadModel.orgName)
            } else {
                notificationCall(FAppLocalString.ediFileUploadFail, ret.msg)
            }
            progressNotificationCall(data.uuid, true)
